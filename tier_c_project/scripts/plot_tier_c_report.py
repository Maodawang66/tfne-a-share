#!/usr/bin/env python
"""从档位 C（tier_c）产物生成实验报告用图 → tier_c_project/figures/"""
...
PRO = ROOT.parent
FIG = PRO / "tier_c_project" / "figures"
ART = ROOT / "artifacts"
TIER = "tier_c"
...
def plot_equity_test():
    eq = pd.read_csv(ART / f"backtest/{TIER}/test/equity_curve.csv")
    eq["trade_date"] = pd.to_datetime(eq["trade_date"], format="%Y%m%d")
    fig, ax = plt.subplots(figsize=(10, 4))
    ax.plot(eq["trade_date"], eq["equity"] / 1e6, color="#16a34a", lw=1.5)
    ax.set_title("档位 C 测试集净值曲线（2025-01 起）")
    ax.set_xlabel("日期")
    ax.set_ylabel("净值（百万元）")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(FIG / "fig1_test_equity.png", dpi=150)
    plt.close(fig)


def plot_fusion_weights():
    w = json.load(open(ART / f"predictions/{TIER}/fusion_weights.json", encoding="utf-8"))["weights"]
    labels = ["TFT-lite\n(时序)", "FT-Transformer\n(截面)", "NewsLoRA\n(新闻)"]
    vals = [w["A"], w["B"], w["C"]]
    colors = ["#2563eb", "#16a34a", "#ca8a04"]
    fig, ax = plt.subplots(figsize=(5, 5))
    ax.pie(vals, labels=labels, autopct="%1.1f%%", colors=colors, startangle=90)
    ax.set_title("三模态 ICIR 融合权重（TFNE-C）")
    fig.tight_layout()
...
def plot_orders():
    orders_dir = ART / "orders" / TIER
    csvs = sorted(orders_dir.glob("orders_*.csv"))
    if not csvs:
        return
    o = pd.read_csv(csvs[-1])
    date = csvs[-1].stem.replace("orders_", "")
    o = o.sort_values("target_weight", ascending=True).tail(15)
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.barh(o["ts_code"], o["target_weight"] * 100, color="#6366f1")
    ax.set_xlabel("目标权重（%）")
    ax.set_title(f"最新调仓 Top15 权重（{date[:4]}-{date[4:6]}-{date[6:8]}）")
...
def plot_ic_bars():
    ic = json.load(open(ART / f"metrics/{TIER}/ic_summary.json", encoding="utf-8"))
    ...
    ax2.set_ylabel("ICIR")
    ax1.set_title("档位 C 融合 score：IC / ICIR 对比")
...
def plot_ic_timeseries():
    ...
    pred = pd.read_parquet(ART / f"predictions/{TIER}/ensemble_scores.parquet")
    ...
    ax.bar(s.index, s.values, width=2, color="#93c5fd", alpha=0.55, label="日度 IC")
    ...
    ax.axvline(pd.Timestamp("2025-01-01"), color="#16a34a", ls="--", lw=1, label="测试集起点")
    ax.set_title("档位 C：日度 Spearman IC（融合 score）")
    ax.set_xlabel("日期")
    ax.set_ylabel("IC")
