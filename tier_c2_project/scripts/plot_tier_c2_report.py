#!/usr/bin/env python
"""从档位 C2（tier_c2）产物生成实验报告用图 → tier_c2_project/figures/"""
...
PRO = ROOT.parent
FIG = PRO / "tier_c2_project" / "figures"
ART = ROOT / "artifacts"
TIER = "tier_c2"
...
def plot_fusion_weights():
    ...
    ax.set_title("三模态 ICIR 融合权重（TFNE-C2）")
    fig.tight_layout()
...
def plot_ic_bars():
    ...
    ax2.set_ylabel("ICIR")
    ax1.set_title("TFNE-C2 融合 score：IC / ICIR 对比")
...
def plot_ic_timeseries():
    pred_path = ART / f"predictions/{TIER}/ensemble_scores.parquet"
    if not pred_path.exists():
        print(f"[skip] {pred_path} 未下载，跳过 IC 时序图")
        return
    ...
    ax.axvline(pd.Timestamp("2025-04-01"), color="#ef4444", ls="--", lw=1, label="验证集起点")
    ax.set_title("TFNE-C2：日度 Spearman IC（融合 score）")
    ax.set_xlabel("日期")
    ax.set_ylabel("IC")
